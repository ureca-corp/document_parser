...
HEADING_STYLE_PATTERNS: dict[str, int] = {
    "outline1": 1, "outline2": 2, "outline3": 3,
    "outline4": 4, "outline5": 5, "outline6": 6,
    "제목": 1, "부제목": 2,
}
...
def _find_children(parent: ET.Element, local_name: str) -> list[ET.Element]:
    """네임스페이스 무관하게 직계 자식 중 local_name과 일치하는 요소를 리스트로 반환한다."""
    return [ch for ch in parent if _strip_ns(ch.tag) == local_name]
...
def _process_element(
    element: ET.Element, elements: list[Paragraph | Table]
) -> None:
    """XML 요소를 재귀적으로 처리하여 Paragraph와 Table을 추출한다."""
    tag = _strip_ns(element)
