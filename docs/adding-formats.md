...
- `extensions()` — 지원하는 파일 확장자 리스트를 반환해요
- `parse(filepath)` — 파일을 파싱해서 `Document`를 반환해요
...
class PdfParser:
    ...
    @staticmethod
    def parse(filepath):
        import pymupdf  # 선택적 의존성은 함수 안에서 import해요
...
- `format_name()` — 포맷 이름을 반환해요 (예: `"html"`)
- `file_extension()` — 파일 확장자를 반환해요 (예: `".html"`)
- `write(doc)` — `Document`를 받아 출력 문자열을 반환해요
